...
export default function TechnoSidebarHeader() {
    ...
    return (
        <div className={`flex items-center w-full transition-all duration-300 ease-in-out ${hovered ? "justify-start pl-4 gap-4" : "justify-center"}`}>
            {hovered && (
                <div className="flex items-center gap-4 transition-opacity duration-500 ease-in-out opacity-100">
                    ...
                    <Avatar className="w-12 h-12 transition-transform duration-300 ease-in-out">
                        <AvatarImage className="rounded-full" src="https://github.com/shadcn.png" alt="User Avatar" />
            ...
            <TechnoIcon type={hovered ? "button" : undefined} className={`transition-transform duration-300 ease-in-out ${hovered ? "ml-auto" : ""}`}>
                <Menu size={32} />
            </TechnoIcon>
        </div>
    );
