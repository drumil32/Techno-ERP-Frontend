...
export default function TechnoRightDrawer({ title, isOpen, onClose, children }: any) {
  return (
    <div
      className={`fixed inset-0 z-50 flex transition-all duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
    >
      {isOpen && <div className="fixed inset-0 bg-opacity-50" onClick={onClose} />}
