...
export default function ShowHistoryDialog({
  ...
  const feeHistory = useQuery<FeeHistoryResponse, Error>({
    ...
    queryFn: (context) => fetchFeeBreakUpHistory(context as QueryFunctionContext<readonly [string, any]>),
  ...
  const feeHistoryData = feeHistory.data?.feeUpdateHistory || [];
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="bg-white sm:min-w-[500px] z-40 p-6 rounded-xl shadow-xl w-full max-w-lg fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
          ...
          {feeHistory.isLoading ? (
            ...
            <>
              <div className={`${showAll ? "max-h-[400px] overflow-y-auto" : ""} space-y-4 px-3 py-2`}>
                {visibleHistory.length ? visibleHistory.map((entry) => (
                  ...
                  >
                    <div className="flex justify-between text-sm text-gray-600">
                      ...
                      <span className="italic">By: <strong>{entry.updatedBy}</strong></span>
                    </div>
                    <div className="mt-2 flex justify-between gap-2 text-sm">
                      <div>
                        <span className="text-gray-500">Net Change:</span><br />
                        <span className={entry.extraAmount >= 0 ? "text-green-600 font-medium" : "text-red-600 font-medium"}>
                          {entry.extraAmount >= 0 ? "+" : "-"}₹ {Math.abs(entry.extraAmount).toLocaleString()}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-500">Updated Fee:</span><br />
                        <span className="text-gray-800 font-medium text-right">₹ {entry.updatedFee.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                )) : (
