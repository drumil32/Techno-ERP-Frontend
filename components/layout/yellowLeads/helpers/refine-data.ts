...
export const refineLeads = (data: any) => {
    const refinedLeads = data.yellowLeads?.map((lead: any, index: number) => ({
        ...
        course: lead.course ?? '-',
        campusVisit: CampusVisitStatus[lead.campusVisit as keyof typeof CampusVisitStatus] ?? lead.campusVisit,  
        finalConversion: FinalConversionStatus[lead.finalConversion as keyof typeof FinalConversionStatus] ?? lead.finalConversion,
